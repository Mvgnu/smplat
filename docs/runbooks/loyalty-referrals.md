...
7. Run `pnpm --filter web test:e2e -- --grep "Loyalty hub"` with the checkout API key unset to execute the storefront redemption happy-path Playwright suite. Extend coverage to assert the activity timeline renders ledger + redemption chips, respects referral/campaign/order filters, and that failed redemptions expose retry controls.
...
10. Run `pnpm --filter web test:e2e -- --grep "Referrals"` without a checkout API key to cover invite creation, share links, cancellation, throttle messaging, and the conversion summary counts rendered in the dashboard card.
